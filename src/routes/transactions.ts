...
import {
    ...
    FilterTransactionsValidationError,
} from "../controllers/transaction-validation.js";
...
transactionRoutes.get("/", async (c) => {
    ...
    if (queryParams.offset)
        parsedParams.offset = parseInt(queryParams.offset, 10);
    if (queryParams.minTotal)
        parsedParams.minTotal = parseInt(queryParams.minTotal, 10);
    if (queryParams.maxTotal)
        parsedParams.maxTotal = parseInt(queryParams.maxTotal, 10);
...
transactionRoutes.get("/:id", async (c) => {
    ...
    const result = await getTransaction(idNum);
